...
test('createToken returns valid token string', function (): void {
...
test('verifyPassword returns UserId on correct password', function (): void {
    $service = app(AuthenticationServiceInterface::class);
...
test('verifyPassword returns null on wrong password', function (): void {
    $service = app(AuthenticationServiceInterface::class);
    $user = User::factory()->create(['password' => Hash::make('password123')]);
...
test('revokeAllTokens deletes all user tokens', function (): void {
    $service = app(AuthenticationServiceInterface::class);
    $user = User::factory()->create();
